...
app = Flask(__name__)
app.secret_key = 'helloouvaissaifithisiisthtesecretkey'
# Config MySQL
mydb = mysql.connector.connect(
    host='db',
    user="root",
    passwd=open('/run/secrets/db-password', 'r').read(),
    database='example',
...
def add_article():
    ...
    if request.method == 'POST' and form.validate():
        ...
        mycursor = mydb.cursor()
        mycursor.execute("INSERT INTO articles (title, body, author) VALUES (%s, %s, %s)",(title, body, 'ouvais-saifi'))
        mydb.commit()
        mycursor.close()
